...
2. **场景化诊断引导**：以可编辑的步骤列表组织诊断场景，每个步骤附有引导文案和说明，用户逐条手动执行，结果展现在同一页面
3. **全命令模式**：支持用户自由输入任何 Arthas 命令，同时提供高危命令正则匹配黑白名单拦截
4. **完整权限管理**：RBAC 角色权限模型，区分管理员/普通操作员/只读用户
5. **操作审计日志**：完整记录每一次诊断操作（含命令原文和执行结果），提供查询
6. **应用运行时日志**：统一的日志配置、输出、脱敏和请求链路追踪，使平台自身具备可观测性

## User Stories

1. 作为 SRE 工程师，我希望能在一个 Web 页面上看到所有配置了 Arthas 的服务器列表，以便快速选择要诊断的目标
2. 作为 SRE 工程师，我希望能选择服务器后输入任意 Arthas 命令并查看执行结果，以便灵活排查问题
3. 作为 SRE 工程师，我想要一个场景化的诊断列表（如"线程死锁检测"），列出逐步操作和引导文案，以便按逻辑顺序完成排查
4. 作为 SRE 工程师，我希望诊断命令的执行结果直接展示在步骤列表中，无需弹窗或切换标签页，以便一屏完成所有操作
5. 作为 SRE 工程师，我希望系统能拦截高危 Arthas 命令，以避免误操作导致线上故障
...
13. 作为所有用户，我希望能使用用户名密码登录系统，登录成功后会话由 HttpOnly Cookie + JWT 维持
14. 作为所有用户，我希望能看到命令执行的历史记录和结果，以便复盘
15. 作为系统管理员，我希望能配置高危命令黑白名单规则（正则表达式），并根据需要启用或禁用
16. 作为系统管理员，我希望能创建/编辑/删除诊断场景和其中的每一个步骤，以便持续积累和优化诊断知识库
17. 作为 SRE 工程师，我希望场景步骤中使用 `{placeHolder}` 占位符标记需要手动填写的参数，并有清晰的提示说明
18. 作为 SRE 工程师，我希望平台的所有认证事件（登录成功/失败、Token 过期、账户锁定）都有运行时日志记录，以便排查登录问题
19. 作为 SRE 工程师，我希望平台的权限拦截事件（角色校验失败、未认证访问）都有 WARN 级别日志，以便发现异常访问行为
20. 作为 SRE 工程师，我希望 Arthas 命令执行的完整链路（接收请求 → CommandGuard 校验 → 发送到 Arthas → 收到响应）都有日志记录，以便追踪命令执行问题
21. 作为 SRE 工程师，我希望高危命令拦截事件有独立的 WARN 日志，以便监控安全风险
22. 作为系统管理员，我希望审计日志写入失败时有 ERROR 日志，以便及时发现审计数据丢失
23. 作为 SRE 工程师，我希望每条日志都包含请求唯一标识（requestId），以便在并发请求中关联同一请求的所有日志
24. 作为 SRE 工程师，我希望日志中自动脱敏 password 和 token 字段，以便日志可以安全地分享和存储
25. 作为系统管理员，我希望生产环境的日志持久化到文件，以便在服务重启后仍可回溯问题
26. 作为系统管理员，我希望日志文件有滚动策略（按大小/日期），以便控制磁盘占用
27. 作为 SRE 工程师，我希望开发环境使用 DEBUG 级别、生产环境使用 INFO 级别，以便在不同环境下获取合适的日志粒度
28. 作为 SRE 工程师，我希望登录请求也包含 requestId 链路追踪，以便排查登录相关的安全问题
29. 作为 SRE 工程师，我希望请求链路追踪在任何异常情况下都能正确清理（不会因认证失败或请求异常导致 MDC 泄漏），以便系统长期稳定运行
30. 作为 SRE 工程师，我希望日志中的客户端 IP 在反向代理场景下仍能获取真实 IP（通过 X-Forwarded-For），以便准确识别请求来源
...
| 角色 | 编码 | 权限范围 |
|------|------|----------|
| 管理员 | ADMIN | 全部功能：服务器 CRUD、用户管理、角色分配、审计日志查看、场景管理、命令守卫配置、执行命令 |
...
| 操作时间 | 请求时间 | 不脱敏 |
| 来源 IP | 请求 IP | 不脱敏 |
| 操作类型 | LOGIN / EXECUTE_COMMAND / MANAGE_SERVER / MANAGE_USER / MANAGE_SCENE / etc. | 不脱敏 |
...
**步骤命令模板：** 支持 `{placeHolder}` 占位符。用户点击执行前可编辑命令框替换占位符为实际值。如：
- 模板: `thread {threadId}`
- 提示: "将上一步输出的线程ID替换 {threadId}，例如 thread 42"
...
1. **线程死锁检测**
   - 步骤1: `thread -b` → 检查死锁线程
   - 步骤2: `thread -n 5` → TOP 5 CPU 活跃线程
   - 步骤3: `thread {threadId}` → 查看具体线程栈

2. **CPU 热点分析**
   - 步骤1: `thread -n 5` → 找出 CPU 最高的线程
   - 步骤2: `thread {threadId}` → 查看线程栈
   - 步骤3: `dashboard -n 1` → 查看当前 JVM 概览

3. **内存泄漏初步检查**
   - 步骤1: `memory` → 查看内存区使用情况
   - 步骤2: `heap -h 10` → 查看堆中大对象
   - 步骤3: `sc -d {className}` → 查看类加载信息

4. **GC 概况诊断**
   - 步骤1: `memory` → 查看内存区
   - 步骤2: `vmtool --action getInstances --className java.lang.management.MemoryPoolMXBean` → 查看各内存池详情

5. **JVM 基础信息**
   - 步骤1: `dashboard -n 1` → JVM 概览
   - 步骤2: `vmoption` → JVM 参数
   - 步骤3: `sysenv` → 系统环境变量
...
| POST | `/api/scene-execute` | 执行场景中的某一步命令 | 已认证 |

### Arthas 执行引擎 (现有模块改造)

**改动点：**
1. 命令发出前经过 `CommandGuard` 拦截校验
2. 执行完成后通过 `@AuditLog` 切面记录审计日志
3. 增加执行超时控制（60s 请求超时，与 Arthas HTTP API 对齐）
4. 同时支持场景单步执行（`/api/scene-execute`）和自由命令执行（`POST /api/execute`）
...
| POST | `/api/execute` | 执行自由命令 | OPERATOR, ADMIN |
| POST | `/api/scene-execute` | 执行场景单步命令 | OPERATOR, ADMIN |
| GET | `/api/scenes` | 场景列表 | 已认证 |
| POST | `/api/scenes` | 创建场景 | ADMIN |
| PUT | `/api/scenes/{id}` | 更新场景 | ADMIN |
| DELETE | `/api/scenes/{id}` | 删除场景 | ADMIN |
...
- **后端**：Java 17 + Spring Boot 3.2.5 + Spring Data JPA + H2 Database + java.net.http.HttpClient + jjwt (JWT 库)
...
└── src/main/
    ...
    │   ├── Application.java
    │   ├── config/
    │   │   ├── MdcFilter.java                # MDC 请求链路追踪 Filter（requestId/username/clientIp）
    ...
    │   │   ├── ArthasServerController.java  # 服务器管理 CRUD (已存在)
    │   │   ├── ArthasExecuteController.java # 命令执行 (已存在)
    │   │   ├── SceneController.java         # 场景 CRUD + 场景执行
    │   │   ├── AuditLogController.java      # 审计日志查询
    │   │   └── CommandGuardController.java  # 高危命令规则管理
    │   ├── entity/
    │   │   ├── ArthasServerEntity.java      # (已存在)
    │   │   ├── SysUser.java
    │   │   ├── SysRole.java
    │   │   ├── SysAuditLog.java
    ...
    │   │   └── SceneStep.java
    │   ├── repository/
    │   │   ├── ArthasServerRepository.java  # (已存在)
    │   │   ├── SysUserRepository.java
    │   │   └── ...
    │   ├── service/
    │   │   ├── AuthService.java             # 登录/认证逻辑
    │   │   ├── UserService.java
    │   │   ├── ArthasServerService.java     # (已存在)
    │   │   ├── ArthasExecuteService.java    # (已存在)
    │   │   ├── SceneService.java
    │   │   ├── AuditLogService.java
    │   │   └── CommandGuardService.java
    │   ├── client/
    │   │   └── ArthasHttpClient.java        # (已存在)
    │   ├── dto/
    │   │   ├── ArthasServerDTO.java         # (已存在)
    │   │   ├── ExecuteRequest.java          # (已存在)
    │   │   ├── ExecuteResponse.java         # (已存在)
    │   │   ├── LoginRequest.java
    │   │   ├── LoginResponse.java
    │   │   └── ...
    │   └── model/
    │       ├── ServerInfo.java              # (已存在)
    │       └── ArthasResponse.java          # (已存在)
    └── resources/
        ├── application.yml
        ├── logback-spring.xml              # Logback 配置（Profile 差异 + 滚动文件 + 脱敏 Converter）
        └── import.sql                       # 初始化数据（admin 用户 + 角色 + 默认规则 + 预置场景）
```
...
| **CommandGuardService** | 正则黑白名单匹配逻辑、优先级校验 | P0 |
...
| **SensitiveDataConverter** | Logback 脱敏 Converter，验证 password/token/Authorization 字段替换正确性，验证非敏感字段不被误脱敏 | P0 |
| **AuthInterceptor MDC** | MdcFilter 在请求处理后 MDC 中存在 requestId 和 username，finally 块后 MDC 被清理；认证失败（preHandle 返回 false）时 MDC 仍被清理；登录请求也有 requestId | P0 |
| **AuthService 日志** | 使用 Logback ListAppender 捕获日志，验证登录成功/失败/锁定场景下的日志级别和内容关键词 | P0 |
...
- **WebSocket 实时推送**：第一版采用同步请求-响应模式，后续可改为 SSE 或 WebSocket 推送实时诊断结果
...
- **步骤结果自动解析抽取实体**：当前使用 `{placeHolder}` 手动替换，后续可做自动解析

## Further Notes

- **安全第一**：Token 在 HttpOnly Cookie 中传输，前端无法读取；高危命令拦截是必选功能
- **分阶段交付**：第一阶段完成安全基础设施（P0），第二阶段实现场景引擎（P1），两阶段互不阻塞
