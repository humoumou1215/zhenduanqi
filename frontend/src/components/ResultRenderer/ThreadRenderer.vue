...
function normalizeThread(thread) {
  return {
    ...
    status: thread.status || thread.threadState || '-',
    cpu: thread.cpu ?? thread.cpuUsage ?? null,
    deltaTime: thread.deltaTime ?? thread.deltaTime ?? null,
    lockedMonitors: thread.lockedMonitors ?? thread.lockedSynchronizers ?? 0,
    threadId: thread.threadId ?? thread.id ?? null,
    stackTrace: thread.stackTrace || null,
  };
}
...
const tableData = computed(() => {
  ...
  if (props.data?.name || props.data?.threadId || props.data?.threadName) {
