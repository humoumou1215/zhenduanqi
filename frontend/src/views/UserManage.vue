    ...
    <el-table :data="users" v-loading="loading" stripe style="width: 100%">
      ...
      <el-table-column label="角色" width="160">
        <template #default="{ row }">
          <el-tag v-for="r in row.roles" :key="r.code" :type="r.code === 'ADMIN' ? 'danger' : r.code === 'OPERATOR' ? 'warning' : 'info'" size="small" style="margin-right: 4px;">
            {{ r.name }}
          </el-tag>
        </template>
      </el-table-column>
      <el-table-column prop="status" label="状态" width="100">
        <template #default="{ row }">
          <el-tag :type="row.status === 'ACTIVE' ? 'success' : 'danger'" size="small">
            {{ row.status === 'ACTIVE' ? '正常' : row.status === 'LOCKED' ? '锁定' : '禁用' }}
          </el-tag>
        </template>
      </el-table-column>
      ...
      <el-form :model="form" label-width="100px">
        ...
        <el-form-item label="角色">
          <el-checkbox-group v-model="form.roleCodes">
            <el-checkbox v-for="r in roleOptions" :key="r.roleCode" :label="r.roleCode">
              {{ r.roleName }}
            </el-checkbox>
          </el-checkbox-group>
        </el-form-item>
    ...
    <el-dialog v-model="pwdDialogVisible" title="重置密码" width="400px">
      <el-form :model="pwdForm" label-width="100px">
        <el-form-item label="新密码">
          <el-input v-model="pwdForm.newPassword" type="password" placeholder="至少8位" show-password />
        </el-form-item>
      </el-form>
      <template #footer>
...
const defaultForm = () => ({ username: '', password: '', realName: '', roleCodes: [], status: 'ACTIVE' })
...
async function handleSave() {
  ...
  try {
    if (isEdit.value) {
      const data = { realName: form.value.realName, status: form.value.status, roleCodes: form.value.roleCodes }
...
async function handleResetPwd() {
  ...
  try {
    await resetPassword(pwdUserId.value, pwdForm.value.newPassword)
